...
class SlidesAccessError(ValueError):
    ...
    def __init__(self, code):
        self.code = (
            ...
            if code
            in {
                "GOOGLE_SLIDES_AUTH_REQUIRED",
                "GOOGLE_SLIDES_ACCESS_DENIED",
                "GOOGLE_SLIDES_API_DISABLED",
            }
            else "GOOGLE_SLIDES_ACCESS_DENIED"
        )
...
def authenticated_slide_text(document_id: str) -> str:
    ...
    parts = [
        (i, "".join(texts(slide)).strip()) for i, slide in enumerate(value.get("slides", []), 1)
    ]
