...
def match_course(name: str, code: str, courses: list[dict]) -> dict | None:
    ...
    if len(matches) > 1:
        raise KnowledgeSyncError(
            "Canvas course matches multiple configured courses (COURSE_MATCH_AMBIGUOUS)",
            code="COURSE_MATCH_AMBIGUOUS",
...
class KnowledgeSync:
    def __init__(
        ...
        if schedule.get("timezone") != "America/Toronto":
            raise KnowledgeSyncError(
                "Course schedule timezone must be America/Toronto (INVALID_TIMEZONE)",
        ...
        if (
            ...
            raise KnowledgeSyncError(
                "Knowledge sync requires an HTTPS Worker URL and sync secret (INVALID_WORKER_CONFIG)",
                code="INVALID_WORKER_CONFIG",
                stage="configuration",
            )
        self.canvas, self.store, self.schedule = canvas, store, schedule
        self.directory, self.url, self.secret, self.session = (
            directory,
            url.rstrip("/"),
            secret,
            session,
    ...
    def _text(self, material: Any) -> str:
        ...
        if material.content_type == "application/pdf":
            ...
            text = "\n\n".join(
                c.text
                for c in extract_powerpoint_text_chunks(material.local_path, max_chars=2_000_000)
            )
        ...
        if not text.strip():
            raise KnowledgeSyncError(
                "A course source has no extractable text (TEXT_EXTRACTION_EMPTY)",
    ...
    def collect(self, context: Any, course: dict) -> list[dict]:
        ...
        for module in api.get_modules():
            visible = attr(module, "published", True) is not False and not attr(
                module, "locked_for_user", False
            )
            for item in module.get_module_items():
                ...
                key = str(
                    attr(item, "page_url", "")
                    if kind == "Page"
                    else attr(item, "external_url", "")
                    if kind == "ExternalUrl"
                    else attr(item, "content_id", "")
                )
                if (
                    not visible
                    or attr(item, "published", True) is False
                    or attr(item, "locked_for_user", False)
                ):
                    ...
                    modules.setdefault((kind, key), []).append(
                        {
                            ...
                            "position": attr(module, "position", None),
        ...
        def allowed(value: Any, kind: str, key: str) -> bool:
            return (
                (kind, key) not in excluded
                and attr(value, "published", True) is not False
                and not any(
                    attr(value, field, False)
                    for field in ("locked_for_user", "hidden_for_user", "hidden", "locked")
                )
            )
        ...
        if syllabus:
            records["syllabus"] = record(
                "syllabus",
                "Course syllabus",
                "syllabus",
                self.canvas._html_to_text(syllabus),
                url=f"{base}/assignments/syllabus",
            )
        pages: dict[str, Any] = {
        ...
        for key, page in pages.items():
            ...
            records[source_id] = record(
                source_id,
                str(attr(full, "title", key)),
                "page",
                text,
                url=f"{base}/pages/{key}",
                updated_at=attr(full, "updated_at", None),
                module=modules.get(("Page", key)),
            )
        files: dict[str, Any] = {}
        try:
            files.update(
        ...
        for (kind, key), item in module_items.items():
            ...
            try:
                ...
                raise KnowledgeSyncError(
                    "A visible module file could not be read (SOURCE_UNREADABLE)",
        ...
        for key, file in files.items():
            ...
            if (
                not key
                or not allowed(file, "File", key)
                or Path(name).suffix.lower() not in {".pdf", ".pptx"}
            ):
                continue
            ...
            if int(attr(file, "size", 0) or 0) > limit:
                ...
                continue
            material = self.canvas._download_lecture_file(
                api, summary, key, name, self.directory, limit, "", None, None
            )
            if material is None:
                raise KnowledgeSyncError(
                    "A published PDF/PowerPoint could not be downloaded safely (SOURCE_UNREADABLE)",
                    code="SOURCE_UNREADABLE",
                    stage="source_retrieval",
                )
            source_id = f"file:{key}"
            records[source_id] = record(
                source_id,
                name,
                "pdf" if name.lower().endswith(".pdf") else "powerpoint",
                self._source_text(material),
                url=material.html_url,
        ...
        for (kind, key), item in module_items.items():
            ...
            try:
                material = self.canvas._download_external_slides(
                    summary,
                    key,
                    str(attr(item, "title", "Lecture slides")),
                    self.directory,
                    "",
                    None,
                    None,
                ...
                records[source_id] = record(
                    ...
                    str(attr(item, "title", "Lecture slides")),
                    "presentation",
                    "Contents not indexed: Google Slides read access is required.",
                    url=key,
                    module=modules.get((kind, key)),
                )
                logging.getLogger("attendr.knowledge_sync").warning(
                    "Linked slides excluded: %s", error.code
                )
                continue
            except (ValueError, OSError, RequestException):
                raise KnowledgeSyncError(
                    "Linked slides could not be read",
                    code="SOURCE_UNREADABLE",
                    stage="source_retrieval",
                ) from None
        ...
        for assignment in api.get_assignments(override_assignment_dates=True):
            ...
            records[source_id] = record(
                ...
                "assignment",
                text,
                url=f"{base}/assignments/{key}",
                updated_at=attr(assignment, "updated_at", None),
                module=modules.get(("Assignment", key)),
                deadline=due,
            )
        for topic in api.get_discussion_topics(only_announcements=True):
            ...
            if posted and datetime.fromisoformat(posted.replace("Z", "+00:00")) > datetime.now(
                timezone.utc
            ):
                continue
            ...
            records[source_id] = record(
                source_id,
                str(attr(topic, "title", "Announcement")),
                "announcement",
                self.canvas._html_to_text(str(attr(topic, "message", "") or "")),
                url=f"{base}/discussion_topics/{key}",
                updated_at=attr(topic, "updated_at", None) or posted,
            )
        days = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"]
        timetable = [
            f"Term: {self.schedule['term']['start_date']} to {self.schedule['term']['end_date']} (America/Toronto)."
        ]
        timetable.extend(
            f"{s['type'].title()}: {days[s['weekday']]} {s['start']}–{s['end']} (America/Toronto)."
            for s in course["sessions"]
        )
        timetable.extend(
            f"No class: {s['start']} to {s['end']} ({s['reason']})."
            for s in self.schedule["term"].get("no_class", [])
        )
        records["verified-schedule"] = record(
            "verified-schedule", "Verified course schedule", "schedule", "\n".join(timetable)
        )
    ...
    def sync(self) -> int:
        ...
        for context in self.canvas._get_active_course_contexts():
            course = match_course(
                context.summary.name, context.summary.course_code or "", self.schedule["courses"]
            )
            if course:
                if course["key"] in seen:
                    raise KnowledgeSyncError(
                        "Multiple active Canvas courses map to the same course key (COURSE_MATCH_AMBIGUOUS)",
                        code="COURSE_MATCH_AMBIGUOUS",
                        stage="course_matching",
                    )
                seen.add(course["key"])
        ...
        for context, course in mapped:
            ...
            try:
                ...
                payload = {
                    "course": {
                        **course,
                        "code": context.summary.course_code or "",
                ...
                for source in records:
                    for offset in range(0, max(1, len(source["chunks"])), 32):
                        ...
                        part["hash"] = hashlib.sha256(
                            json.dumps(
                                {k: v for k, v in part.items() if k != "hash"}, sort_keys=True
                            ).encode()
                        ).hexdigest()
                        parts.append(part)
                if len(parts) > 20_000:
                    raise KnowledgeSyncError(
                        "Course snapshot exceeds source limit (SNAPSHOT_TOO_LARGE)",
        ...
        if failures:
            raise KnowledgeSyncError(
                f"{len(failures)} course snapshot(s) failed; previous complete snapshots preserved: "
                + "; ".join(failures)
            )
        if not count:
            raise KnowledgeSyncError(
                "No configured active Canvas courses were found (NO_MATCHING_COURSES)",
    ...
    def _upload(self, action: str, payload: dict) -> None:
        ...
        for attempt in range(3):
            ...
            try:
                response = self.session.post(
                    f"{self.url}/api/knowledge/{action}",
                ...
                code = (
                    ...
                    if status in (401, 403)
                    else "WORKER_RATE_LIMITED"
                    if status == 429
                    else "WORKER_SERVER_ERROR"
                    if status >= 500
                    else "WORKER_REJECTED"
                )
                if status < 500 and status != 429:
                    raise KnowledgeSyncError(
                        "Worker rejected the course snapshot",
                        code=code,
                        stage=stage,
                        status=status,
                        retries=attempt,
        ...
        raise KnowledgeSyncError(
            "Course snapshot upload failed", code=code, stage=stage, status=status, retries=attempt
        )
