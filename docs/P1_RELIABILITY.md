...
- `extraction_cache`: content-addressed extraction results plus post-lecture summary records. Summary cache identity includes the source-bundle hash, prompt version, and model. The validated payload and source provenance are stored before Discord delivery, so retrying a failed delivery does not regenerate or re-download material.
