...
COMMAND = {
    "name": "ask",
    "type": 1,
    "description": "Ask a question about one of your courses",
    "options": [
        {
            "name": "question",
            "description": "For my web development course, when’s my midterm?",
            "type": 3,
            "required": True,
            "min_length": 1,
            "max_length": 1000,
...
def setup(token: str, application_id: str, guild_id: str, owner_id: str, session=requests) -> str:
    def call(method: str, path: str, **kwargs):
        response = session.request(
            method, f"{API}{path}", headers={"Authorization": f"Bot {token}"}, timeout=20, **kwargs
        )
        if not response.ok:
            try:
    ...
    if existing:
        ...
        channel = call(
            ...
            json={
                "name": "ask",
                "type": 0,
                "topic": "Ask Attendr about your courses with /ask and one natural-language question.",
                "permission_overwrites": [
                    {"id": guild_id, "type": 0, "deny": "1024", "allow": "0"},
                    {
                        "id": owner_id,
                        "type": 1,
                        "allow": str(1024 | 2048 | 65536 | 2147483648),
                        "deny": "0",
                    },
...
def main(argv=None) -> int:
    ...
    if any(not os.getenv(k, "").strip() for k in required):
        print(
            ...
            + ", ".join(k for k in required if not os.getenv(k, "").strip()),
            file=sys.stderr,
        )
        return 1
    if not all(
    ...
    if not args.upload_worker_secrets:
        print(
            "Upload DISCORD_ASK_CHANNEL_ID and GEMINI_API_KEY to Worker secrets before using /ask."
        )
    return 0
