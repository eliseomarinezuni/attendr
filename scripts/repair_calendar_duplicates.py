...
def managed_events(service, calendar_id):
    ...
    while True:
        ...
        events.extend(
            ...
            if e.get("extendedProperties", {}).get("private", {}).get("attendr_source") == "canvas"
        )
...
def main():
    ...
    backup = (
        ROOT
        / "data"
        / ("calendar-duplicates-" + datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ") + ".json")
    )
    backup.write_text(
        json.dumps({"source": source, "destination": target, "events": duplicates}, indent=2)
    )
    backup.chmod(0o600)
    for event in duplicates:
        current = (
            service.events()
            .get(calendarId=target, eventId=destinations[uid(event)]["id"])
            .execute()
        )
        if current.get("status") == "cancelled" or uid(current) != uid(event):
            raise SystemExit("Destination changed; cleanup stopped.")
        service.events().delete(
            calendarId=source, eventId=event["id"], sendUpdates="none"
        ).execute()
