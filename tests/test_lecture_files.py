...
def public_dns(monkeypatch, mapping=None):
    ...
    monkeypatch.setattr(
        socket,
        "getaddrinfo",
        lambda hostname, *_args, **_kwargs: dns_result(*mapping.get(hostname, ("93.184.216.34",))),
    )
...
def test_stream_rejects_insecure_redirect(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        requests.Session,
        "get",
        Mock(return_value=response([], 302, {"Location": "http://unsafe.test"})),
    )
...
@pytest.mark.parametrize(
    "url", ["http://public.test/file", "https://user:password@public.test/file"]
)
...
def test_google_slides_cache_and_login_page_rejection(tmp_path, monkeypatch):
    ...
    assert download.call_count == 1
    c.state_store = None
    download.side_effect = lambda url, dest, limit: dest.write_text(
        "<!DOCTYPE html><html>Sign in</html>"
    )
    with pytest.raises(ValueError, match="unavailable"):
        c._download_external_slides(*args)
    assert (
        c._download_external_slides(NS(id=1), "https://unrelated.test", "", tmp_path, "", 1, 1)
        is None
    )
...
def test_numbered_week_labels_match_correct_session(tmp_path):
    ...
    session = next(
        ...
        if s.course_key == "example-web" and s.weekday == 1 and s.activity == "lecture"
    )
    runner = LectureQuizRunner(
        None, None, None, schedule, materials_directory=tmp_path, state_path=tmp_path / "state.db"
    )
    common = dict(
        course_name="Example Web Systems", module_name="Basics", module_position=2, updated_at=None
    )
    ...
    due = schedule.ended_lecture_sessions(
        datetime(2026, 9, 17, 22, tzinfo=schedule.timezone), retry_hours=336
    )
...
def test_introduction_module_matches_only_first_course_lecture(tmp_path):
    ...
    sessions = sorted(
        (
            s
            for s in schedule.sessions
            if s.course_key == "example-graphics" and s.activity == "lecture"
        ),
...
def test_topic_modules_map_by_canvas_order_and_bundle_same_module(tmp_path):
    ...
    common = dict(course_id=1, course_name="Example Graphics", updated_at=None)
    intro = NS(
        uid="intro",
        source_id="1",
        title="Welcome",
    ...
    selected = runner._select_materials(
        sessions[1], ended, (rendering, modeling_b, intro, modeling_a)
    )
...
def test_topic_module_order_fails_closed_when_positions_are_ambiguous(tmp_path):
    ...
    session = next(
        s
        for s in schedule.sessions
        if s.course_key == "example-graphics" and s.weekday == 2 and s.activity == "lecture"
    )
    runner = LectureQuizRunner(
        None, None, None, schedule, materials_directory=tmp_path, state_path=tmp_path / "state.db"
    )
    common = dict(course_id=1, course_name="Example Graphics", updated_at=None, item_position=1)
    one = NS(
        uid="one",
        source_id="1",
        title="Vectors",
...
def test_explicit_session_mapping_overrides_module_order(tmp_path):
    ...
    root = Path(__file__).resolve().parents[1]
    data = json.loads((root / "data/course_schedule.example.json").read_text())
    data["courses"][0]["lecture_materials"]["sessions"] = {"2026-09-09": {"module_id": "60"}}
...
def test_stable_material_mapping_survives_title_and_position_changes(tmp_path):
    ...
    ended = datetime(2026, 9, 9, 11, tzinfo=schedule.timezone)
    material = NS(
        uid="canvas:lecture-file:1:99",
        source_id="99",
        title="Renamed Topic",
        module_name="Moved Topic",
        module_position=99,
        item_position=8,
        module_id="60",
        item_id="601",
...
def test_private_slides_falls_back_to_authorized_reader(tmp_path, monkeypatch):
    ...
    c.state_store = None
    monkeypatch.setattr(
        "academic_assistant.canvas_client.stream_download",
        Mock(side_effect=LectureDownloadError(401)),
    )
    ...
    result = c._download_external_slides(
        NS(id=1, name="Web"),
        "https://docs.google.com/presentation/d/abc/edit",
        "01a",
        tmp_path,
