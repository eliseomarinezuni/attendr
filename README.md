...
The default run sends unseen announcements, extracts dated items from syllabuses and all recent announcements, syncs deadlines plus configured institution dates and every lecture/lab/tutorial in the private timetable, and sends a configurable digest (72 hours by default). Discord quiet hours default to midnight through 9:00 AM in `APP_TIMEZONE`; every notifier route fails closed before enqueueing or posting during that window. Canvas assignments override announcements; newest announcements override syllabuses. A date without a stated time remains an all-day event on its stated date. Academic date ranges use an exclusive next-day end. The exception is a same-course midterm or exam on an unambiguous lecture date: when the source omits the time, Attendr uses that lecture slot. Explicit exam times are preserved when replacing a lecture occurrence.
...
The free Cloudflare Worker under `worker/` stays online when the computer is off. Every five minutes it checks D1 for sessions that are starting and posts buttons in `#study-sessions`: **Session complete**, **Reschedule**, and **Task complete**. Automated Worker reminders use the same midnight–9:00 AM Toronto quiet window; user-initiated Discord buttons and `/ask` responses remain available. Completion removes the corresponding Calendar block; task completion removes all remaining blocks for that task. The Python planner and Worker share only opaque IDs and session metadata through an authenticated endpoint.
...
Calendar authorization is intentionally repaired only on a trusted local machine. Run `.venv/bin/python scripts/setup_google.py --reauthorize` to force a new offline refresh token, verify it with `.venv/bin/python scripts/setup_google.py --check`, then replace the `GOOGLE_TOKEN_B64` repository secret with `base64 < token.json | tr -d '\n'`. The always-online study buttons use a separate copy of the same Google OAuth credentials in Cloudflare; refresh and verify that copy with `.venv/bin/python scripts/configure_cloudflare_secrets.py --google-only`. Never paste token values into an issue or chat. If refresh tokens repeatedly expire, inspect **Google Cloud Console → Google Auth Platform / OAuth consent screen → Publishing status**. OAuth applications left in Testing can receive short-lived refresh tokens; evaluate the appropriate production publishing status for this private app. Publishing does not automatically remove Google's verification requirements.
